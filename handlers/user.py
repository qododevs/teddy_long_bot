...
async def cmd_start(message: Message):
    ...
    await message.answer(
        ...
        "Ну что, начнём это цирковое представление под названием «твоя жизнь»? Я готов к хлопушкам и барабанам. Просто напиши мне *Привет* и мы начнем бешеные гонки по твоей жизни",
        reply_markup=kb
    )
...
async def clear_context(message: Message, session: AsyncSession):
    ...
    await message.answer("(΄◉◞౪◟◉｀) виртуально потирает лапки.\n\n"
        ...
        "Ну че встал в ступоре? Я жду твой первый (снова) жизненный провал! (ﾉ◕ヮ◕)ﾉ*:･ﾟ✧", reply_markup=None)
...
async def stop_response(message: Message, state: FSMContext):
    ...
    await message.answer("🛑 Так-так-так... Прервали на полуслове. Видимо, моя гениальная мысль была слишком для тебя тяжела.\n"
            "Ладно, твоя взяла. Спасайся, пока я не передумал и не начал сначала. (¬_¬)", reply_markup=None)
